...
public class CertificateService {
    ...
    private List<CommentDto> getCommentDtos(List<CommentEntity> commentEntities) {
        return commentEntities.stream()
                .map(commentEntity -> {
                    ...
                    if (comment == null) {
                        throw new EntityNotFoundException("Comment with ID " + commentEntity.getId() + " does not exist");
                    }
    ...
    private List<CommentEntity> createComments(List<CommentDto> commentDtos, CertificateEntity certificate) {
        return commentDtos.stream()
                .map(commentDto -> {
                    UserEntity commentUser = userRepository.findById(commentDto.getUserId());
                    if (commentUser == null) {
                        throw new EntityNotFoundException("User with ID " + commentDto.getUserId() + " does not exist");
                    }
