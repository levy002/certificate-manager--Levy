...
public class SearchQueryUtil {
  ...
  public static <T> void addCaseInsensitiveLikePredicate(
      List<Predicate> predicates, CriteriaBuilder cb, Root<T> root, String field, String value) {
    if (value != null && !value.trim().isEmpty()) {
      predicates.add(cb.like(cb.lower(root.get(field)), "%" + value.trim().toLowerCase() + "%"));
    }
  }

  public static <T> T findByField(
      Class<T> entityClass, String field, String value, EntityManager entityManager) {
